...
Description:	: WPF controller to access Azure IoT-hub to onnect and monitor devices to analyze untapped data.
Copyright       : GMedia-IT-Consulting 
email           : oleg_gorlov@yahoo.com
Date            : 05/05/2017
Release         : 1.0.0
Comment         : 
				: Implementation Universal Windows Platform March 2017 Released 3/7/2017
...
namespace SmartProSystems
{
    ...
    internal class HttpInterfaceManager : IDisposable
    {
        ...
        //public MainPage mm = new MainPage();
        ...
        private const string HostKey = "xxxIoT-Hub.azure-devices.net";

        private const string DeviceKey = "xxx";

        //--- xxxIoT-Hub / Settings / Hostname / Shared access policies / host / Primary key

        private const string SharedKey = "xxx";


        private AzureIoTHub msgHub;

        public static string Window_Left_Status = "";
        public static string Text_GpioStatus = "";
        ...
        private async Task SendMessages(int numMsg)
        {
            ...
            for (int i = 0; i < numMsg; i++)
            {
                if (sendMsg)
                {
                    ...
                    Window_Left_Status = MainPage.Text_ButtonStatus_.ToString();
                    Text_GpioStatus = MainPage.Text_GpioStatus_.ToString();

                    //--- MainPage mm = new MainPage();
                    //string mm1 = mm.Status.ToString();

                    var led = new
                    {
                        LED_PIN_5 = MainPage.LED_GPIO05,
                        Window_Left_Status = Window_Left_Status,
                        Text_GpioStatus = Text_GpioStatus
                    };
                    ...
                    try
                    {
                        Debug.WriteLine("msgHub.SendDeviceToCloudMessageAsync : "+ fullMsg);
